...
public class LoggerServerTest {
    ...
    public void testRollViewLog() throws IOException {
        ...
        String resultTmpDemoString = this.logClientService.rollViewLog(
                "localhost", PropertyUtils.getInt(Constants.LOGGER_RPC_PORT),"/tmp/demo.txt", 0, 1000);
    ...
    public void testRemoveTaskLog() throws IOException {
        ...
        Boolean b = this.logClientService.removeTaskLog("localhost", PropertyUtils.getInt(Constants.LOGGER_RPC_PORT),"/tmp/remove.txt");

        Assert.assertTrue(b);

        String result = this.logClientService.viewLog("localhost", PropertyUtils.getInt(Constants.LOGGER_RPC_PORT),"/tmp/demo.txt");
