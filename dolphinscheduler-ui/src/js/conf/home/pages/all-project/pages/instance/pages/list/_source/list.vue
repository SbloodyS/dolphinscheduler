          ...
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              ...
              <div slot="reference" class="name-wrapper">
                <router-link :to="{ path: `/projects/${scope.row.projectCode}/instance/list/${scope.row.id}` , query:{code: scope.row.processDefinitionCode}}" tag="a" class="links"><span class="ellipsis">{{ scope.row.name }}</span></router-link>
              </div>
            </el-popover>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _gantt (item) {
        this.$router.push({ path: `/projects/${item.projectCode}/instance/gantt/${item.id}` })
      },
      _topCheckBoxClick (v) {
