...
<template>
  <div class="shell-model">
    <!-- ip -->
    <m-list-box>
      <div slot="text">{{ $t('ip') }}</div>
      <div slot="content">
        <div style="display: inline-block;">
          <el-select
            v-model="ip"
            :disabled="isDetails"
            @change="_handleIpChanged"
            style="width: 150px;"
            size="small">
            <el-option
              v-for="ip in clickhouseIPList"
              :key="ip.ip"
              :value="ip.ip"
              :label="ip.ip">
            </el-option>
...
<script>
  ...
  import mRelatedEnvironment from '../_source/relatedEnvironment.vue'
  import mWorkerGroups from '../_source/workerGroups.vue'
  import { clickhouseIPList } from './_source/commcon'
  ...
  export default {
    ...
    data () {
      return {
        ...
        sql: '',
        ip: '',
        clickhouseIPList: clickhouseIPList,
        // Custom parameter
        localParams: [],
        // resource(list)
        resourceList: [],
